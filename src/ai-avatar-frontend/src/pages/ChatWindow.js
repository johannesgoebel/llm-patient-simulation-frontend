...
const ChatWindow = () => {
  ...
  const fetchCurrentPromptData = async () => {
    try {
      const response = await fetch('http://llm-patient-simulation-backend.vercel.app/conversation/current_prompt', {
        method: 'GET',
        headers: {
  ...
  const handleButtonClick = async (name) => {
    try {
        const url = `http://www.llm-patient-simulation-backend.vercel.app/conversation/vignette?name=${encodeURIComponent(name)}`;
  ...
  const fetchSystemPromptData = async () => {
    try {
      const response = await fetch('http://www.llm-patient-simulation-backend.vercel.app/conversation/load_case_vignette', {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
        },
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const url = `http://www.llm-patient-simulation-backend.vercel.app/retrieve_answer?message=${encodeURIComponent(input)}&api_key=${apiKey}`;
